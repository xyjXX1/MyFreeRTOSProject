...
void WifiConnect(void)
{
	...
	while (1)
	{
		...
		{
            pNetDev->isConnected = 1;
			printf("Connect WIFI ok\r\n");
			break;
		}
		else
		{
            pNetDev->isConnected = 0;
...
void GetNetTime(void)
{
    ...
    while(1)
    {
        ...
        if(pNetDev && pNetDev->isConnected==1)
        {
            pNetDev->GetTime(pNetDev, &time);
            /* 清除屏幕 */
            memset(ptDevice->FBBase, 0, ptDevice->iSize);
            /* 显示年月日 */
            ShowTextInDisplayDevice(ptDevice, 16, 31, time.year);
            ShowTextInDisplayDevice(ptDevice, 48, 31, "/");
            ShowTextInDisplayDevice(ptDevice, 56, 31, time.month);
            ShowTextInDisplayDevice(ptDevice, 80, 31, "/");
            ShowTextInDisplayDevice(ptDevice, 88, 31, time.day);
            
            /* 显示时分秒 */
            ShowTextInDisplayDevice(ptDevice, 24, 48, time.hour);
            ShowTextInDisplayDevice(ptDevice, 40, 48, ":");
            ShowTextInDisplayDevice(ptDevice, 48, 48, time.min);
            ShowTextInDisplayDevice(ptDevice, 64, 48, ":");
            ShowTextInDisplayDevice(ptDevice, 72, 48, time.sec);
        }
